<template>
	<view>
		
		<view class="zhanw"></view>
		<view class="header flex flex-y-center">
			<view class="" style="width: 50rpx;" @tap="back">
				<tui-icon name="arrowleft" color="#fff"></tui-icon>
			</view>
			<view class="title" style="font-size: 15pt;">
				<text>地址管理</text>
			</view>
		</view>
		
		<view class="content">
			<view class="list flex">
				<!-- :class="row.is_default==1? 'head_active': ''" -->
				<view class="row flex"  :key="index" @tap="select(row)">
					<view class="left flex">
						<view class="head flex-y-center" >
							01
						</view>
					</view>
					<view class="center flex">
						<view class="name-tel flex">
							<view class="name">阿森</view>
							<view class="tel">13249597474</view>
							<view class="default" >
								默认
							</view>
						</view>
						<view class="address">
							广州市白云区机场路388号
						</view>
					</view>
					<view class="right">
						<view class="icon bianji" @tap.stop="edit(row)">
							
						</view>
					</view>
				</view>
			</view>
		</view>
		<view class="add flex">
			<view class="btn flex flex-y-center" @tap="add">
				新增地址
			</view>
		</view>
	</view>
</template>
<script>
	export default {
		data() {
			return {
				isSelect:false,
				addressList:[],//收货地址信息
				head:[]//收货人的姓
			};
		},
		onShow() {
			
			uni.getStorage({
				key:'delAddress',
				success: (e) => {
					let len = this.addressList.length;
					if(e.data.hasOwnProperty('id')){
						for(let i=0;i<len;i++){
							if(this.addressList[i].id==e.data.id){
								this.addressList.splice(i,1);
								break;
							}
						}
					}
					uni.removeStorage({
						key:'delAddress'
					})
				}
			})
			uni.getStorage({
				key:'saveAddress',
				success: (e) => {
					let len = this.addressList.length;
					if(e.data.hasOwnProperty('id')){
						for(let i=0;i<len;i++){
							if(this.addressList[i].id==e.data.id){
								this.addressList.splice(i,1,e.data);
								break;
							}
						}
					}else{
						let lastid = this.addressList[len-1];
						lastid++;
						e.data.id = lastid;
						this.addressList.push(e.data);
					}
					uni.removeStorage({
						key:'saveAddress'
					})
				}
			})
		},
		onLoad(e) {
			if(e.type=='select'){
				this.isSelect = true;
			}
			this.$request({
					url: this.$api.user.address_list,
			}).then(res => {
				this.addressList = res.data.list;
				for(let i=0;i<this.addressList.length;i++){
					let temp = this.addressList[i].name;
					this.head.push(temp.substring(0,1));
				}
			});
		},
		methods:{
			back(){
				uni.navigateBack()
			},
			edit(row){
				uni.setStorage({
					key:'address',
					data:row,
					success() {
						uni.navigateTo({
							url:"edit/edit?type=edit"
						})
					}
				});
				
			},
			add(){
				uni.navigateTo({
					url:"edit/edit?type=add"
				})
			},
			select(row){
				//是否需要返回地址(从订单确认页跳过来选收货地址)
				if(!this.isSelect){
					return ;
				}
				uni.setStorage({
					key:'selectAddress',
					data:row,
					success() {
						uni.navigateBack();
					}
				})
			}
		}
	}
</script>

<style lang="scss" scoped>
	.zhanw {
		width: 100%;
		height: var(--status-bar-height);
	}
.header {
	height: 100rpx;
	width: 100vw;
	position: fixed;
	top: 0;
	left: 0;
	background-image: url('/static/img/home/header.png');
	background-size: 100%;
	.title {
		width: 100%;
		color: #fff;
		justify-content: center;
		padding-right: 60rpx;
		text-align: center;
	}
}	
	
.content {
	width: 100vw;
	height: 100vh;
	padding-top: 100rpx;
	box-sizing: border-box;
}
	
view{
	
}
	.icon {
		// &.bianji {
		// 	&:before{content:"\e61b";}
		// }
		// &.tianjia {
		// 	&:before{content:"\e81a";}
		// }
	}
	.add{
		position: fixed;
		bottom: 0;
		width: 100%;
		height: 120upx;
		justify-content: center;
		align-items: center;
		.btn{
			box-shadow: 0upx 5upx 10upx rgba(0,0,0,0.4);
			width: 70%;
			height: 80upx;
			border-radius: 80upx;
			background-color: #134AFF;
			color: #fff;
			justify-content: center;
			align-items: center;
			.icon{
				height: 80upx;
				color: #fff;
				font-size: 30upx;
				justify-content: center;
				align-items: center;
			}
			font-size: 30upx;
		}
	}
	.list{
		flex-wrap: wrap;
		background-color: #FFFFFF;
		.row{
			width: 96%;
			padding: 20upx 2%;
			border-bottom: 1px solid #EEEEEE;
			.left{
				width: 90upx;
				flex-shrink: 0;
				align-items: center;
				.head{
					width: 70upx;
					height: 70upx;
					background:linear-gradient(to right,#ccc,#aaa);
					color: #fff;
					justify-content: center;
					align-items: center;
					border-radius: 60upx;
					font-size: 35upx;
				}
				.head_active{
					background-color: #134AFF;
				}
			}
			.center{
				width: 100%;
				flex-wrap: wrap;
				.name-tel{
					width: 100%;
					align-items: baseline;
					.name{
						font-size: 34upx;
					}
					.tel{
						margin-left: 30upx;
						font-size: 24upx;
						color: #777;
					}
					.default{

						font-size: 22upx;
						
						background-color: #f06c7a;
						color: #fff;
						padding: 0 18upx;
						border-radius: 24upx;
						margin-left: 20upx;
					}
				}
				.address{
					width: 100%;
					font-size: 24upx;
					align-items: baseline;
					color: #777;
				}
			}
			.right{
				flex-shrink: 0;
				align-items: center;
				margin-left: 20upx;
				.icon{
					justify-content: center;
					align-items: center;
					width: 80upx;
					height: 60upx;
					border-left: solid 1upx #aaa;
					font-size: 40upx;
					color: #777;
				}
			}
		}
	}
</style>
