	...
	export default {
		...
		methods: {
			...
			doLogin(){
				...
				this.$request({
					url: this.$api.passport.phone_login,
					method: 'POST',
					data: {
						binding:this.phoneNumber,
						sms_code: this.code
					}
				})
			...
			getCode(){
				...
				if(!(/^1(3|4|5|6|7|8|9)\d{9}$/.test(self.phoneNumber))){ 
					uni.showToast({title: '请填写正确手机号码',icon:"none"});
					return false; 
				} 
				...
				self.$request({
					url: self.$api.passport.send_sms_code,
					method: 'POST',
					data: {
						binding: self.phoneNumber
					}
				}).then(res => {
